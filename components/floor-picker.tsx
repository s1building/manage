...
export function FloorPicker({ value, onChange, label = "작업층 확인" }: { value: string; onChange: (value: string) => void; label?: string }) {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="flex max-h-[90dvh] max-w-lg flex-col overflow-hidden p-4 sm:p-6"><DialogHeader><DialogTitle>{label}</DialogTitle></DialogHeader><p className="text-sm text-muted-foreground">원하는 층을 하나씩 눌러 선택하거나 해제하세요.</p><Button type="button" variant="secondary" className="w-full" onClick={() => setDraft([...FLOOR_OPTIONS])}>전층선택</Button><div className="grid max-h-[55vh] grid-cols-3 gap-2 overflow-y-auto overscroll-contain px-1 pb-2 select-none touch-pan-y sm:grid-cols-4">{FLOOR_OPTIONS.map((floor) => { const selected = draft.includes(floor); return <button key={floor} type="button" onClick={() => toggle(floor)} className={`min-h-11 rounded-lg border px-1 text-sm font-semibold transition-colors ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:bg-muted"}`}><span className="flex items-center justify-center gap-1">{selected && <Check className="size-3.5" />}{floor}</span></button> })}</div><DialogFooter><Button type="button" onClick={() => { onChange(draft.join(", ")); setOpen(false) }}>선택 완료 ({draft.length})</Button></DialogFooter></DialogContent></Dialog>
  </>
}
